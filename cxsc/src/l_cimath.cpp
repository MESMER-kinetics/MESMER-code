...
/* CVS $Id: l_cimath.cpp,v 1.2 2007/05/16 14:59:00 cxsc Exp $ */
...
// Blomquist, 28.11.2006;
    ...
    interval re(rez);  // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    real x(mid(re));
    l_interval u,v;  // moeglicherweise genuegt auch: interval u,v;
...
std::list<l_cinterval> sqrt_all( const l_cinterval& z )
{
	...
	w = l_cinterval( interval( resxl, resxu ), interval( resyl, resyu ) );
    }
    else
