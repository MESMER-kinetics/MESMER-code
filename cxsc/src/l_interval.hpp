...
/* CVS $Id: l_interval.hpp,v 1.13 2007/05/22 15:26:30 cxsc Exp $ */
...
//      friend inline l_interval_Sup Sup (l_interval &)  throw();
      //! Returns the infinum of an interval
    ...
    // Calculating expo(x[k]) of the greatest |x[k]|.
      ...
      //! Returns the interval with the new given infinum value
      ...
      //! Returns the interval with the unchecked new given infinum value
      ...
      friend inline bool point_intv(const l_interval &a );  // bool delivers: a is a point interval;
      //! Checks if the argument is zero
      friend inline bool zero_(const l_interval& li) throw(); // Blomquist,27.11.02
      //! Multiplication of interval with \f$ 2^n \f$
      friend inline void times2pown(l_interval& x, 
                               int n) throw();    // Blomquist,28.11.02
      //! Multiplication of interval with \f$ 2^n \f$
      friend inline void Times2pown(l_interval& x, 
                               int n) throw();    // Blomquist,28.03.07
      friend void l_realz2l_interval(const l_real& lr, const interval& z,
			       l_interval& li) throw(); // Blomquist,28.11.02
