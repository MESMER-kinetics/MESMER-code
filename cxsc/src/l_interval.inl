...
/* CVS $Id: l_interval.inl,v 1.8 2007/05/22 15:26:30 cxsc Exp $ */
...
// Multiplication of a with 2^n;
{
    int fac,rest;
    if (n>=0)
    {
	fac = n/LI_maxexpo1;  rest = n%LI_maxexpo1;
	for (int k=1; k<=fac; k++)
	...
	times2pown(a,rest);
       ...
       if (n<-2100)
       {
          if (0<a) a = l_interval(-minreal,minreal);
          else if (Inf(a)>=0) a = l_interval(0,minreal);
               else a = l_interval(-minreal,0);
       } else
       {
	  fac = n/LI_Min_Exp_;  rest = n%LI_Min_Exp_;
	  for (int k=1; k<=fac; k++)
