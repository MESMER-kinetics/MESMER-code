...
/* CVS $Id: real.hpp,v 1.21 2007/05/16 14:59:03 cxsc Exp $ */
...
/*!
The arithmetic of C-XSC is based on the IEEE standard for binary floating-point arithmetic. Data of the type real
consist of all floating-point numbers and special values specified by the standard for floating-point numbers of double mantissa
length. Therefore, a number of the type real is a 64-bit value, and the base \f$ b \f$ of the floating-point system \f$ R = R(b , l , e_min, e_max)\mbox{ is }2 \f$.

The C-XSC data type real differs only in some special aspects such as error handling from the C type double if used on a IEEE
standard conforming arithmetic. If the C compiler on the host computer is not standard conforming, the data type real uses its
own IEEE software arithmetic. Hence, the introduction of a new data type is necessary for the portability of C-XSC.
...
The most significant bit is the sign bit, denoted by \f$ s \f$ . If it is one, the non-zero floating-point number is negative. Otherwise
the floating-point number is positive.

The remaining 63 bits of the floating-point numbr are subdivided as follows:

\subsection mantissa Mantissa

The mantissa length \f$ l \f$ of a real value is 53 bits. Bit 53 is not explicitly stored. Its value depends upon the normalization
or denormalization of the floating-point number represented. With this mantissa length \f$ l \f$ , decimal numbers
with  a maximum of 15 fractional digits can be represented with high accuracy. Since a binary floating-point format is
used, it is impossible to store all decimal numbers exactly. For example, the decimal number 0.6 is not exactly representable
as a binary floating-point number. However, we can read decimal constants with controlled rounding into
a variable of the type real.

\subsection exponent Exponent

The exponent \f$ e \f$ of the floating-point number is stored in a field of 11 bit width using a biased notation. A value of
1023 is subtracted from the stored value to get the actual exponent value. This yields \f$ e_{min} = -1022 \f$ and \f$ e_{max} = 1023 \f$ .
...
inline double _double(const real &a) throw() { return a.w; }
/*!
\deprecated use standard contructors for typecasting

\sa cxsc::real::real(const double &a)
*/
inline real   _real(const double &a) throw() { return real(a); }
...
inline real Max(const real & a, const real & b) { return a>b?a:b; } 
  // for compatibility with CToolbox library - from former r_util.hpp
...
inline real pow2n(const int n) throw(); // returns 2^n; Blomquist 03.10.02

// ----  special constants -----------------------------------------
//! Smallest normalized representable floating-point number
extern const real MinReal;    // kleinste positive normalisiert darstellbare
                              // Gleitkommazahl
//! Smallest positive denormalized representable floating-point number
extern const real minreal;    // kleinste positive denormalisiert darstellbare
                              // Gleitkommazahl, Blomquist, 26.09.02.
//! Greatest representable floating-point number
extern const real MaxReal;    // groesste darstellbare Gleitkommazahl
