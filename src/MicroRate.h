...
namespace mesmer
{
    ...
    The derived concrete classes are plugin classes: new classes can be added without
    changing any of the existing code. They have a single global instance,
    the constructor of which registers the class with the base class. Subsequently,
    a pointer to the class is obtained by supplying the id (a string) to the Find function.
    **/
    ...
    public:
        ...
        virtual bool calculateMicroRateCoeffs(Reaction* pReact, std::vector<double> &kfmc) = 0 ;
        virtual bool testMicroRateCoeffs(Reaction* pReact, std::vector<double> &kfmc, PersistPtr ppbase) const;
