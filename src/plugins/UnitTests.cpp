...
namespace mesmer
{
    ...
    private:
        ...
        bool Test_NormalDistribution() const;
    ...
    bool UnitTests::DoCalculation(System* pSys)
    {
        ...
        // Test Normal random number class.
        status = (status && Test_NormalDistribution());
    ...
    bool UnitTests::Test_NormalDistribution() const {

        bool status(true);

        ctest << endl;
        underlineText("Test: Normal random number method.");

        Sobol sobol;
        const size_t rnsize(5);
        size_t nSamples(10000);
        double width(0.1);

        double mean(0.0), var(0.0);
        vector<double> rndmd(rnsize, 0.0);
        vector<double> distribution(101, 0.0);
        long long seed2(0);
        for (size_t i(0); i < nSamples; i++) {
            sobol.sobol(rndmd.size(), &seed2, rndmd);
            for (size_t j(0); j < rndmd.size(); j++) {
                double x = NormalCDFInverse(rndmd[j]);
                mean += x;
                var += x * x;
                int l = nint(x / width) + 50;
                if (l <= 100 && l >= 0)
                    distribution[l] += 1.0;
            }
        }
        mean /= double(nSamples * rnsize);
        var /= double(nSamples * rnsize);
        var -= mean * mean;

        ctest << endl;
        underlineText("Mean and Variance (Sobol driven)");
        ctest << "  Mean:    " << formatFloat(mean, 5, 12) << endl;
        ctest << "  Variance:" << formatFloat(var, 5, 12) << endl;
        ctest << endl;

        double x(-5.0);
        for (size_t i(0); i < distribution.size(); i++, x += 0.1) {
            ctest << formatFloat(x, 5, 12) << formatFloat(distribution[i], 5, 12) << endl;
        }
        ctest << endl;
