...
namespace {
  ...
  public:
	...
	virtual void rndLocation(const vector<double> &rndmd, const vector<double> &currentLoc, vector<double> &locData, vector<double> &newLoc) = 0 ;
  ...
  public:
	...
	virtual void rndLocation(const vector<double> &rndmd, const vector<double> &currentLoc, vector<double> &locData, vector<double> &newLoc) {
	  locData = rndmd ;
  ...
  public:
	...
	double orthFn(size_t order, const double x) {
	  double x0 = x/m_rtDet ;
	  double f  = exp(-x0*x0/2.0)*m_hfMap[order](x0)/m_rtDet ; 
	  return f ; 
	} ;
	bool initializeDist(System* pSys, string &err) {
	  ...
	  if (m_CorrelMtx = ReadMatrix<double>(pp)) { 
		m_rtDet = sqrt(m_CorrelMtx->Determinant()) ;
	...
	void rndLocation(const vector<double> &rndmd, const vector<double> &currentLoc, vector<double> &locData, vector<double> &newLoc) {
...
namespace mesmer
{
  ...
  bool SensitivityAnalysis::DoCalculationOld(System* pSys)
  {
	...
	for (size_t nCnd(0) ; nCnd < nConditions ; nCnd++) {
	  ...
	  for (size_t itr(1) ; itr <= m_nSample ; itr++) {
		vector<double> rndmd(m_nVar,0.0), dummy(m_nVar,0.0) ;
		sobol.sobol(rndmd.size(), &seed, rndmd) ;

		// Use random vector generated by sobol method to perturb parameter values.

		m_probDensity->rndLocation(rndmd, currentLocation, dummy, newLocation) ;
  ...
  bool SensitivityAnalysis::DoCalculationNew(System* pSys)
  {
	...
	for (size_t nCnd(0) ; nCnd < nConditions ; nCnd++) {
	  ...
	  Table outputData(m_nOut, vector<double>(m_nSample, 0.0)) ;
	  ...
	  for (size_t itr(0) ; itr < m_nSample ; itr++) {
		...
		m_probDensity->rndLocation(rndmd, originalLocation, locationData[itr], newLocation) ;
