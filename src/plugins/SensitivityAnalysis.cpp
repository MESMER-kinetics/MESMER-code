...
namespace mesmer
{
  ...
  private:
	...
	// This method calculates the legendre expansion coefficients by MC integration.
	bool CalcExpansionCoeffs(const vector<vector<double> > &locationData, const vector<vector<double> > &outputData, vector<vector<double> > &alphas, vector<vector<double> > &betas) ;

	// This method calculates the legendre expansion coefficients by MC integration.
	// Variance reduction is used to improve convergence.
	bool CalcExpansionCoeffsVarRed(const vector<vector<double> > &locationData, const vector<vector<double> > &outputData, vector<vector<double> > &alphas, vector<vector<double> > &betas) ;
	...
	for (size_t nCnd(0) ; nCnd < nConditions ; nCnd++) {
	  ...
	  for (size_t itr(0) ; itr < m_maxIterations ; itr++) {
		...
		if (!f0Initialized) {
		  ...
		  outputData.resize(m_nOut, vector<double>(m_maxIterations, 0.0)) ;
	  ...
	  vector<vector<double> > betas(m_nOut,  vector<double>(m_nVar*m_nVar*m_order*(m_order - 1)/2, 0.0) ) ;

	  // CalcExpansionCoeffs(locationData, outputData, alphas, betas) ;
	  CalcExpansionCoeffsVarRed(locationData, outputData, alphas, betas) ;
  ...
  // This method calculates the legendre expansion coefficients by MC integration.
  bool SensitivityAnalysis::CalcExpansionCoeffs(const vector<vector<double> > &locationData, const vector<vector<double> > &outputData, vector<vector<double> > &alphas, vector<vector<double> > &betas) {
  ...
  // This method calculates the legendre expansion coefficients by MC integration.
  // Variance reduction is used to improve convergence.
  bool SensitivityAnalysis::CalcExpansionCoeffsVarRed(const vector<vector<double> > &locationData, const vector<vector<double> > &outputData, vector<vector<double> > &alphas, vector<vector<double> > &betas) {
	...
	for (size_t nOut(0) ; nOut < m_nOut ; nOut++) {
	  ...
	  for (size_t iVred(0) ; iVred < 5 ; iVred++) {
		...
		for (size_t j(0) ; j < alpha0.size() ; j++) {
		  cinfo << alpha[j] << " " << nrmlFctr*alpha0[j] << endl ;
